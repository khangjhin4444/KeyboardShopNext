...
import { ProductSkeleton } from "../../home/_components/product_skeletion";
...
export default function Page() {
  ...
  const keyword = param.keyword as string;
  const {
    data,
    ...
    queryFn: ({ pageParam = 1 }) =>
      ProductUsecase.getSearchProduct(keyword!, pageParam),
      ...
      {isLoading ? (
        ...
        <div>
          <div
            className={clsx(
              ` gap-8 mt-10 `,
              data?.pages.length !== 1 && data?.pages[0].length! >= 4
                ? "grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4"
                : "flex justify-center",
            )}
          >
            {data?.pages[0].length == 0 && (
              <h2>No Product! Try another Keyword.</h2>
            )}
            {data?.pages[0].length !== 0 &&
