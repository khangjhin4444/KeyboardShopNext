...
export default function ProductGrid({
  ...
  const {
    ...
    queryKey: ["infinite-products-category", type, sub, sort],

    queryFn: ({ pageParam = 1 }) =>
      ProductUsecase.getProductsCategory({
        type,
        page: pageParam,
        sort,
        sub: sub,
      ...
      {!inView || isLoading ? (
        ...
        <div>
          {data?.pages?.[0]?.length === 0 ? (
            <div className="text-center text-gray-500 py-20 mt-10 border-2 border-dashed rounded-lg text-xl font-semibold">
